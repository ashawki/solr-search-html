...
//    }
    protected Parser getParser(Metadata metadata, ParseContext context) {
        return getParser(MediaType.parse(metadata.get(Metadata.CONTENT_TYPE)), context);
    }

    public Parser getParser(MediaType type) {
    	return getParser(type, new ParseContext());
    }

    protected Parser getParser(MediaType type, ParseContext context) {
        Map<MediaType, Parser> map = getParsers(context);
        //MediaType type = MediaType.parse(metadata.get(Metadata.CONTENT_TYPE));
        if (type != null) {
           // We always work on the normalised, canonical form
